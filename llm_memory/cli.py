...
def _install_lmc_globally(project_root: Path) -> bool:
    """Write ~/.local/bin/lmc. Returns True if that dir is already on PATH."""
    import os
    import stat
    ...
    lmc_script.write_text(
        f"#!/usr/bin/env bash\n"
        f"# lmc — LLM Memory Compiler (generated by lmc init)\n"
...
def init(agent: str | None, provider: str | None, knowledge_dir: str | None, daily_dir: str | None) -> None:
    ...
    # ── Install lmc globally ──
    lmc_in_path = _install_lmc_globally(project_root)
    lmc_target = Path.home() / ".local" / "bin" / "lmc"
    _console.print(f"  [green]✓[/green] Wrote [cyan]{lmc_target}[/cyan]")
    if not lmc_in_path:
        _console.print(
            "  [yellow]![/yellow] Add [cyan]~/.local/bin[/cyan] to your PATH to use [bold]lmc[/bold] globally:\n"
